...
const themes = {
    classic: { name: '經典藍白', primary: '#2563eb', bg: '#f8fafc', sidebar: '#ffffff', card: '#ffffff', text: '#1e293b', border: '#e2e8f0' },
    dark: { name: '深邃星空', primary: '#3b82f6', bg: '#0b0f1a', sidebar: '#111827', card: '#1f2937', text: '#f1f5f9', border: '#374151' },
    ocean: { name: '深海湛藍', primary: '#0ea5e9', bg: '#082f49', sidebar: '#0c4a6e', card: '#115e7a', text: '#f0f9ff', border: '#0e7490' },
    sunset: { name: '夕陽橙金', primary: '#f59e0b', bg: '#fffbeb', sidebar: '#ffffff', card: '#ffffff', text: '#78350f', border: '#fef3c7' },
    rose: { name: '玫瑰嫩粉', primary: '#db2777', bg: '#fff1f2', sidebar: '#ffffff', card: '#ffffff', text: '#881337', border: '#ffe4e6' }
}
...
const formatUID = (index) => String(index).padStart(4, '0')

const loadUsers = () => {
    const defaultAccount = [
        { email: 'admin', name: '管理者', role: 'admin', uid: '0000', totalSpent: 0, transactions: 0 },
        { email: 'user', name: '測試者', role: 'user', uid: '0001', totalSpent: 45800, transactions: 15 }
    ];
...
<template>
  <div class="admin-scope" :style="{ backgroundColor: currentStyle.bg, color: currentStyle.text }">
    ...
    <aside class="sidebar" :style="{ backgroundColor: currentStyle.sidebar, borderColor: currentStyle.border }">
        <div class="brand">
            ...
            <div class="brand-text">
                ...
                <span :style="{ color: currentStyle.primary }">CONTROL PANEL</span>
            </div>
        </div>
        ...
        <div class="sidebar-footer">
            <button class="logout-btn" @click="handleLogout">安全登出 🚪</button>
        </div>
    </aside>

    <main class="main-content">
        <header class="top-bar" :style="{ background: currentStyle.sidebar, borderBottom: `1px solid ${currentStyle.border}` }">
            ...
            <div class="admin-online">
                <span class="pulse-dot"></span> 管理員：線上
            </div>
        </header>
        ...
        <div class="scroll-viewport">
            ...
            <div class="stats-row">
                <div v-for="s in [
                    ...
                    <div class="stat-icon-large" :style="{ background: s.c + '15', color: s.c }">{{ s.i }}</div>
                </div>
            </div>

            <div class="glass-panel" :style="{ backgroundColor: currentStyle.card, borderColor: currentStyle.border }">
                
                <div v-if="activeTab === 'analytics'" class="tab-pane">
                    <h3 class="pane-title">消費權重即時排行</h3>
                    <table class="modern-table">
                        <thead>
                            <tr :style="{ color: currentStyle.text + '80', borderBottom: `2px solid ${currentStyle.primary}30` }">
                                <th>排名</th><th>管理權限</th><th>累計流量</th><th>交易次</th><th>均值</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr v-for="(u, i) in topUsers" :key="u.uid" class="table-row">
                                ...
                                <td style="opacity: 0.7">{{ formatCurrency(u.avgSpent) }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                ...
                <div v-if="activeTab === 'api'" class="tab-pane">
                    <h3 class="pane-title">核心 API 節點協議</h3>
                    <div class="api-grid">
                        <div v-for="api in apiServices" :key="api.id" 
                            ...
                            <div class="api-head">
                                <span class="name" :style="{ color: currentStyle.text }">{{ api.name }}</span>
                ...
                <div v-if="activeTab === 'users'" class="tab-pane">
                    <div class="search-bar">
                        <input v-model="searchQuery" placeholder="🔍 搜尋用戶名稱或 UID..." class="m-input" :style="{ background: currentStyle.bg, color: currentStyle.text, borderColor: currentStyle.border }">
                    </div>
                    <table class="modern-table">
                        <thead>
                        ...
                        <tbody>
                            <tr v-for="u in filteredUsers" :key="u.uid" class="table-row">
                                ...
                                <td>
                                    <select v-model="u.role" :disabled="u.email === 'admin'" class="m-select" :style="{ background: currentStyle.bg, color: currentStyle.text }">
                                ...
                                <td>
                                    ...
                                    <span v-else class="lock-label">🔒 核心鎖定</span>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                ...
                <div v-if="activeTab === 'system'" class="tab-pane">
                    ...
                    <div class="theme-grid">
                        <div v-for="(style, id) in themes" :key="id" 
                            ...
                            :style="{ border: currentTheme === id ? `2px solid ${style.primary}` : `1px solid ${currentStyle.border}` }"
                            @click="setTheme(id)">
                            <div class="theme-preview" :style="{ background: style.bg }">
                                <div class="p-side" :style="{ background: style.sidebar }"></div>
                                <div class="p-accent" :style="{ background: style.primary }"></div>
                            </div>
                            <p class="theme-name" :style="{ color: currentTheme === id ? style.primary : '' }">{{ style.name }}</p>
                        </div>
                    </div>
                </div>
...
<style scoped>
/* 限定在 admin-scope 以內的排版 */
.admin-scope { display: flex; width: 100%; height: 100vh; overflow: hidden; position: fixed; top: 0; left: 0; }
...
.logo-box { width: 44px; height: 44px; border-radius: 12px; display: flex; align-items: center; justify-content: center; color: white; font-weight: 800; font-size: 20px; }
.brand-text h2 { font-size: 22px; font-weight: 700; margin: 0; }
.brand-text span { font-size: 10px; font-weight: 700; letter-spacing: 1px; margin-top: 4px; display: block; }

.nav-item { width: 100%; border: none; background: none; padding: 16px 24px; text-align: left; cursor: pointer; font-size: 15px; font-weight: 500; display: flex; align-items: center; gap: 12px; transition: 0.2s; }
...
.top-bar { height: 64px; display: flex; justify-content: space-between; align-items: center; padding: 0 40px; flex-shrink: 0; }
.pulse-dot { display: inline-block; width: 8px; height: 8px; border-radius: 50%; background: #10b981; box-shadow: 0 0 8px #10b981; margin-right: 8px; }

/* 內容滾動區：避免溢出捲軸 */
.scroll-viewport { padding: 32px 40px; overflow-y: auto; flex: 1; }
...
.stat-icon-large { width: 56px; height: 56px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 30px; }

/* 主面版 */
.glass-panel { border-radius: 24px; padding: 40px; border: 1px solid; min-height: 500px; box-shadow: 0 10px 30px rgba(0,0,0,0.03); }
...
.rank-badge { color: white; padding: 4px 12px; border-radius: 8px; font-weight: 800; font-size: 12px; }
...
.metrics { display: flex; justify-content: space-between; font-size: 12px; padding: 12px; background: rgba(0,0,0,0.03); border-radius: 10px; margin-top: 15px; }

.theme-grid { display: grid; grid-template-columns: repeat(5, 1fr); gap: 16px; }
.theme-card { padding: 14px; border-radius: 20px; cursor: pointer; text-align: center; transition: 0.3s; }
.theme-preview { height: 70px; border-radius: 12px; position: relative; overflow: hidden; border: 1px solid rgba(0,0,0,0.1); margin-bottom: 10px; }
.p-side { width: 22px; height: 100%; position: absolute; left: 0; }
.p-accent { width: 10px; height: 10px; position: absolute; bottom: 8px; right: 8px; border-radius: 50%; }

/* 控件 */
.m-input { padding: 12px 18px; border-radius: 12px; border: 1px solid; outline: none; width: 340px; margin-bottom: 24px; font-size: 14px; }
.m-select { padding: 6px 12px; border-radius: 8px; border: 1px solid rgba(0,0,0,0.1); cursor: pointer; }
.btn-del { color: #ef4444; border: 1px solid #ef4444; background: none; padding: 8px 16px; border-radius: 8px; cursor: pointer; font-weight: 700; }
.lock-label { font-size: 11px; font-weight: 800; opacity: 0.5; }
</style>